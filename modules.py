...
class path_UNI(nn.Module): 

    def __init__(
            self, model_name="UNI", pretrained=CFG.pretrained, trainable = CFG.trainable
            ):
        super().__init__()
        self.model = timm.create_model("hf-hub:MahmoodLab/UNI", 
                                       pretrained=True, 
                                       init_values=1e-5, 
...
class ImageEncoder_resnet152(nn.Module):
    ...
    def __init__(
        self, model_name='resnet152', pretrained=CFG.pretrained, trainable=CFG.trainable
    ):
        super().__init__()
        self.model = timm.create_model(
            model_name, pretrained, 
            pretrained_cfg_overlay=dict(file='/data/lab_ph/matt/SGCL2ST/original/model.safetensors'),
            num_classes=0, global_pool="avg"
        )
        for p in self.model.parameters():
            p.requires_grad = trainable
...
class CONCH(nn.Module): 
    ...
    def __init__(
            self, model_name="CONCH", pretrained=CFG.pretrained, trainable = CFG.trainable
            ):
        super().__init__()
        self.model, self.preprocess = create_model_from_pretrained("conch_ViT-B-16", 
                                                         checkpoint_path="/data/lab_ph/matt/SGCL2ST/conch/checkpoints/conch/pytorch_model.bin", 
    ...
    def forward(self,x):
        ...
        x = x.unsqueeze(0)
        print(x.shape, "X SHAPE IN IMAGE ENCODER UNI")
            #self.model.eval()
        img_emb = self.model.encode_image(x)
        return img_emb
        

class path_giga(nn.Module):
    def __init__(
            self, model_name="prov-gigaPath", pretrained=CFG.pretrained, trainable = CFG.trainable
            ):
        super().__init__()
...
def ZINB_loss(x, mean, disp, pi, scale_factor=1.0, ridge_lambda=0.0):
    ...
    print(num_gpu, "number of gpu detected insize zinb loss")
    eps = 1e-10
    print(len(mean), "leanth mean inside zinb")
    if isinstance(scale_factor,float):
        print("scale is a float and i am in the loop")
        scale_factor=np.full((len(mean)),scale_factor)
    scale_factor = scale_factor[:, None]
    print(mean.shape, "mean shape")
    print(scale_factor.shape, "scale shape")
